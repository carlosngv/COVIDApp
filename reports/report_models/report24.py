...
# 24
def cases_tests_comparison_country(df, variables, problem):
    ...
    tests_df = len(country_selected_df[tests_param])

    # Storing days count
    for i in range (0, infected_df):
        days_count.append(i)

    for i in range (0, tests_df):
    ...
    Y2 = country_selected_df[tests_param][0:days_to_predict + 1]
    plt.scatter(X, Y)
    ...
    plt.title('Comparación tendencia en Casos/Pruebas en ' + country.upper() + '\n' + title)
    plt.xlabel('Días')
    plt.ylabel('Data')
    plt.legend(('Casos', 'Pruebas'), loc='upper right')
    if prediction_value > prediction_value2:
        description = ''' \n
        La tendencia de casos en el paìs de {} para los siguientes {} días indica que el numero de
        vacunados tiende a decrecer conforme pasan los días en comparación a las pruebas de vacunación, que crecen de una forma más lenta.
        '''.format(country, days_to_predict)
    else:
        description = ''' \n
        La tendencia de personas vacunadas en el paìs de {} para los siguientes {} días indica que el numero de
        pruebas tiende a decrecer conforme pasan los días en comparación a la confirmación de nuevos casos, que crecen de una forma más lenta.
        '''.format(country, days_to_predict)

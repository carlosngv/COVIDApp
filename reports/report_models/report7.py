...
# 7
def infection_trend_country_byday(df, variables, problem):
    ...
    country_param = variables[3] # Country parameter name
    # Preparing graph
    ...
    df = df.groupby(['date_ordinal', country_param], as_index=False)[target].sum()
    ...
    if coef < 0:
        ...
        El cuál indica que el numero de infectados ha disminuido conforme pasan los días en {}. MSE = {}; RMSE = {}; R2 = {}
        '''.format(coef, country, round(mse,2),round(rmse,2),round(r2,2))
    else:
        description = ''' \n
        El coeficiente de regresión tiene un valor de: {}. \n

        El cuál indica que el numero de infectados ha incrementado conforme pasan los días en {}.  MSE = {}; RMSE = {}; R2 = {}
        '''.format(coef, country, round(mse,2),round(rmse,2),round(r2,2))

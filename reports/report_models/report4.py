...
# 4
def mortality_prediction_department(df, variables, problem):
    ...
    country = variables[2]
    department = variables[3]
    days_to_predict = int(variables[4])
    degree_number = int(variables[5])

    # Fitlering by department
    df = df.loc[df['Pais'] == country]
    department_selected_df = df.loc[df['Departamento'] == department]
    deaths_df = len(department_selected_df[target])
    ...
    if initial_prediction_value > prediction_value:
        description = ''' \n
        La predicción de muertes en el departamento de {} en el país de {} para los siguientes {} días es de {} e indica que el numero de
